...
export default function LandingPage() {
  ...
  return (
    <div className={`w-full min-h-screen flex flex-col ${showLogin || showSignUp ? 'bg-white opacity-90' : ''}`}>
      {/* Navbar */}
      <nav className="w-full flex justify-between items-center px-6 md:px-12 py-4 bg-white border-b border-gray-300 fixed top-0 left-0 right-0 z-50">
        <div className="flex items-center space-x-2 cursor-pointer">
          <img src="/assets/bloghive-text-logo.png" alt="Logo" className="w-36 md:w-40 h-auto" />
        </div>
      ...
      {/* Hero Section */}
      <div className="relative flex-1 flex flex-col md:flex-row items-center justify-center text-center md:text-left px-6 md:px-16 py-24 md:py-32  text-white mt-16 md:mt-20">
        <div className="md:w-1/2">
          <h2 className="text-8xl md:text-6xl font-bold mb-4 leading-tight text-black">Human Stories & Ideas</h2>
          <p className="text-xl md:text-2xl font-medium mb-6 text-black">A place to read, write, and deepen your knowledge.</p>
          <button className="px-6 py-3 bg-green-700 text-white text-lg rounded-full hover:bg-green-800 cursor-pointer " onClick={() => setShowLogin(true)}>
            Start Reading
          </button>
        </div>
      ...
      {/* Footer */}
      <footer className="w-full py-6 px-6 md:px-12 bg-gray-100 text-gray-700 text-center text-sm md:text-md font-medium flex flex-col md:flex-row justify-between items-center space-y-3 md:space-y-0">
